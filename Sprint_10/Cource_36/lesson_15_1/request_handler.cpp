...
const json::Node RequestHandler::PrintRoute(const json::Dict& request_map) const {
    ...
    result["request_id"s] = request_map.at("id"s).AsInt();
    if (!catalogue_.FindRoute(route_number)) {
        result["error_message"s] = json::Node{ static_cast<std::string>("not found"s) };
    }
    else {
        result["curvature"s] = GetBusStat(route_number)->curvature;
        result["route_length"s] = GetBusStat(route_number)->route_length;
        result["stop_count"s] = static_cast<int>(GetBusStat(route_number)->stops_count);
        result["unique_stop_count"s] = static_cast<int>(GetBusStat(route_number)->unique_stops_count);
    }
