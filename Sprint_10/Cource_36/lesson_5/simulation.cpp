...
void RunSimulation(std::vector<std::unique_ptr<Person>>& people, int num_days) {
...
void Tests() {
    struct TestObserver : public PersonObserver {
        ...
        std::optional<int> old_satisfaction;
        std::optional<int> new_satisfaction;
...

    {
        ...
        std::unique_ptr<Person> p = std::make_unique<DestructivePerson>(person_destroyed);
...
int main() {
    ...
    people.emplace_back(std::make_unique<Worker>("Mario"s, 35));
    people.emplace_back(std::make_unique<Student>("Harry Potter"s, 18));
    ...
    for (auto& person : people) {
        std::cout << person->GetName() << "' satisfaction: "s << person->GetSatisfaction() << ", dance count: "s
             << person->GetDanceCount() << std::endl;
    }
